...
from scripts.pricing.providers import bfl, decart, krea, recraft, riverflow
...
def test_media_manifests_match_runtime_fixed_price_contract() -> None:
    ...
    for provider in ("recraft", "bfl", "decart", "nscale", "krea", "riverflow"):
...
def test_media_providers_are_refreshable_prepaid_gateway_routes() -> None:
    expected = {"recraft", "bfl", "decart", "nscale", "krea", "riverflow"}
    ...
    riverflow_model = riverflow.MODEL_ID
    assert riverflow_model in IMAGE_MODEL_ID_SET
    riverflow_manifest = json.loads((MANIFEST_DIR / "riverflow.json").read_text())
    riverflow_image = next(
        row for row in riverflow_manifest["models"] if row["id"] == riverflow_model
    )
    if riverflow_image.get("routable") is False:
        assert riverflow_model not in models
        assert f"{riverflow_model}@riverflow/prepaid" not in endpoints
    else:
        assert riverflow_model in models
        assert f"{riverflow_model}@riverflow/prepaid" in endpoints
