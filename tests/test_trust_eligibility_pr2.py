...
def arm_store(store: Any, db: Any) -> Settings:
    ...
    for provider in ("stripe", "x402", "owner_inventory"):
        ...
        db.typed.setdefault("tr_trust_backfill", {})[(provider, marker.account_id, "test", marker.source, marker.source_version)] = row
    return settings
...
def test_gate_other_preconditions(condition: str, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    settings = arm_store(store, db)
    workspace_state(db)
    marker = db.typed["tr_trust_backfill"][("x402", "acct_1", "test", "stripe-created-lists", "stripe-trust-v1")]
