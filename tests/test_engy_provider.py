...
def test_engy_parser_preserves_exact_prices_models_and_cache_rates() -> None:
    ...
    assert set(prices) == {"z-ai/glm-5.2", "qwen/qwen3.6-35b-a3b"}
    ...
    assert (
        prices["qwen/qwen3.6-35b-a3b"].tiers[0].prompt_cached_micro_per_m
        == 15_000
    )
...
def test_engy_manifest_routes_use_exact_upstream_ids_and_customer_prices() -> None:
    endpoints = [
        endpoint for endpoint in MODEL_ENDPOINTS.values() if endpoint.provider == "engy"
    ]
