...
1. **NEVER raw-DML / ad-hoc-script prod billing tables.** Use tested tooling only: `scripts/deploy/ramp_typed_billing.py` (`audit`, `pause`, `unpause`, `repair`), `scripts/grant_credit.py`, and merged store methods. The historical JSON seed/reconcile commands are deleted. An ad-hoc `settle_atomic` loop was correctly **blocked by the safety classifier** — don't repeat it. If you need a new prod operation, add it as a reviewed CLI subcommand first.
