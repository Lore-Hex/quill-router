...
sides are derived, not hardcoded:

- **writes** — every construction of `EncryptedSecretEnvelope` anywhere under
  `src/trusted_router`, following module-level and local aliases, subclasses,
  and `dataclasses.replace`, and refusing outright when an envelope's
  `.algorithm` is assigned after construction or the constructor is handed to
  something else as a value;
- **accepts** — `enclave-go/internal/byokcache/accepted_formats.json` **at the
  ...
  every non-test `.go` file in that package, and the gate verifies each one at
  the same commit, so a declaration cannot outlive the package it describes.

so a future v3 is covered without editing the checker.

**The read side used to parse the case labels out of the switch in
`envelopeAAD`, and that was wrong.** A case label is not acceptance: an
adversarial review produced four compiling, gofmt-clean edits that keep
`case AlgorithmV2:` in `cache.go` while rejecting v2 — an erroring case body
behind a feature flag, a kill-switch guard ahead of the switch, a rejection in
`decryptEnvelope` before `envelopeAAD` is called, and a renamed live dispatch
with the old function left as dead code — and all four passed the gate. Each is
the exact outage this section describes. Behaviour is now the only evidence
accepted, which is why the declaration is generated by a round trip rather than
...
further up the enclave would read as acceptance. And it blocks during a bind
window rather than reasoning about it: two enclave builds behind one hostname
with one published commit means one of them is unverified, and §4.0's own
instruction is to roll the enclave out fully first.
...
did not check, and the run now says so instead.

**Known state as of this change:** all three clouds block. `aws-release.json`
and `azure-release.json` carry no `source_commit`; every currently released
enclave commit, GCP's included, predates
`enclave-go/internal/byokcache/accepted_formats.json`, so there is no measured
answer to read at those commits. That is the designed fail-closed behaviour, not
a bug: the previous green on GCP came from parsing case labels, which is the
evidence this change stopped accepting. Both blocks clear the same way — publish
a record naming a commit (quill-cloud-proxy's
...
generated declaration. No `source_commit` was backfilled, because the commit
