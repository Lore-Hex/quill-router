...
def run_provider_backfill(
    ...
    if not account_id or environment not in PROVIDER_ENVIRONMENTS[provider] or not history_start < drained_at <= end:
...
def run_provider_recurring(
    ...
    if not provider_marker_qualifies(marker, provider=provider, account_id=account_id, environment=environment):
