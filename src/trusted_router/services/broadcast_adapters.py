...
from trusted_router.storage import BroadcastDestination, Generation
...
class WebhookOTLPBroadcastAdapter:
    type = "webhook"
...
_ADAPTERS: dict[str, BroadcastAdapter] = {
    PostHogBroadcastAdapter.type: PostHogBroadcastAdapter(),
    WebhookOTLPBroadcastAdapter.type: WebhookOTLPBroadcastAdapter(),
}


def adapter_for(destination_type: str) -> BroadcastAdapter | None:
    return _ADAPTERS.get(destination_type)


def supported_destination_types() -> set[str]:
...
def _decrypt_api_key(destination: BroadcastDestination, settings: Settings) -> str:
    ...
    return decrypt_control_secret(
        ...
        purpose=_secret_context(destination.id, "api_key"),
    )
...
def _decrypt_headers(destination: BroadcastDestination, settings: Settings) -> dict[str, str]:
    ...
    raw = decrypt_control_secret(
        ...
        settings,
        workspace_id=destination.workspace_id,
        purpose=_secret_context(destination.id, "headers"),
    )
...
def _secret_context(destination_id: str, kind: str) -> str:
