...
def tier_cap(settings: Settings, tier: int) -> int:
    return (
        0, settings.spend_lease_tier1_cap_microdollars,
...
def spend_cap(settings: Settings, tier: int | None) -> int:
    ...
    ceiling = max(settings.spend_lease_max_microdollars,
                  settings.spend_lease_tier3_cap_microdollars)
...
class LeaseTrustState:
    ...
    def refusal(self, *, now: datetime, max_age_seconds: int) -> str | None:
        ...
        if not reconciliation_is_fresh(
            self.reconciled_through, now=now, max_age_seconds=max_age_seconds,
        ):
...
def trust_gate_failure(
    store: Any, settings: Settings, *, reader: Any, now: datetime,
) -> str | None:
    from trusted_router.storage_trust_reconciliation import MARKER_COLUMNS, _marker_from_row

    if (settings.storage_backend not in {"spanner-bigtable", "spanner-clickhouse"}
    ...
    if not account.startswith("acct_") or len(account) <= 5:
        return "provider_not_configured"
    requirements = [
        MarkerRequirement(provider, account, settings.environment,
                          STRIPE_TRUST_SOURCE, STRIPE_TRUST_SOURCE_VERSION)
        for provider in sorted(providers)
    ]
    requirements.append(MarkerRequirement(
        OWNER_INVENTORY_PROVIDER, OWNER_INVENTORY_ACCOUNT_ID, settings.environment,
        OWNER_INVENTORY_SOURCE, OWNER_INVENTORY_SOURCE_VERSION,
    ))
    markers = [_marker_from_row(row) for row in reader.execute_sql(
    ...
    for requirement in requirements:
        ...
        if (marker is None or marker.completed_at is None or marker.completed_at > now
                or marker.history_start > marker.closed_through):
            return "marker_incomplete"
        if requirement.provider != OWNER_INVENTORY_PROVIDER:
            if (marker.consistency_delay_seconds < STRIPE_CONSISTENCY_DELAY_SECONDS
                    or settings.trust_reconcile_max_age_seconds < (
                        marker.consistency_delay_seconds
                        + 2 * settings.trust_reconcile_interval_seconds)):
                return "consistency_delay"
            if not reconciliation_is_fresh(
                marker.closed_through, now=now,
...
def lease_eligibility(
    store: Any, settings: Settings, workspace_id: str | None = None,
    *, reader: Any = None, now: datetime | None = None,
    ...
    if failure:
        ...
        ops_alert(f"trust.gate_unarmed condition={failure}",
                  fingerprint=["trust.gate_unarmed", failure])
    ...
    shards = list(reader.execute_sql(
    ...
    if (account is None or sorted(int(row[0]) for row in shards)
            != list(range(credit_shard_count(account)))):
        return None, "reconciliation_stale"
    state = read_lease_trust(reader, store._param_types, workspace_id)
    if state is None:
        return None, "reconciliation_stale"
    return state.tier, state.refusal(now=now, max_age_seconds=settings.trust_reconcile_max_age_seconds)
