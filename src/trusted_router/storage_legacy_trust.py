...
def postgres_pause(conn: Any, workspace_id: str) -> tuple[bool, int]:
    ...
    return (
        any(str(row[0] or "") not in {"", "[]"} for row in rows),
...
def recover_released_postgres(conn: Any, workspace_id: str, store: Any) -> None:
    ...
    for event_id, recovered, debt in rows:
        balance = conn.execute(
            ...
            "WHERE workspace_id = %s AND shard = 0 FOR UPDATE",
            (workspace_id,),
            prepare=False,
        ).fetchone()
        if balance is None:
        ...
        conn.execute(
            ...
            (
                int(recovered or 0) + take,
                int(debt) - take,
                "debited" if take == int(debt) else "partial",
                workspace_id,
                event_id,
            ),
            prepare=False,
    ...
    remaining = conn.execute(
        "SELECT event_id FROM tr_trust_event WHERE workspace_id = %s "
        "AND kind = 'payment' AND unrecovered_micro > 0",
        (workspace_id,),
    ).fetchall()
...
def create_spanner_legacy_authorization(
    keys: Any,
    authorization: Any,
    index_id: str | None,
    *,
    ...
    def create(tx: Any) -> Any:
        prior = (
            io.read_entity_tx(tx, "gateway_authorization_idempotency", index_id, dict)
            if index_id
            else None
        )
        ...
        if prior and prior.get("authorization_id"):
            existing = io.read_entity_tx(
                tx, "gateway_authorization", str(prior["authorization_id"]), GatewayAuthorization
            )
            if existing is not None:
                return existing
        epoch = spanner_pause_epoch(tx, io.param_types, authorization.workspace_id)
        if billing_paused_tx(tx, io.param_types, authorization.workspace_id) or (
            expected_pause_epoch is not None and epoch != expected_pause_epoch
        ):
            ...
            if index_id:
                io.write_entity_tx(
                    tx, "gateway_authorization_idempotency", index_id, {"reason": "billing_paused"}
                )
            return None
...
def spanner_pause_epoch(reader: Any, pt: Any, workspace_id: str) -> int:
...
def legacy_pause_epoch(store: Any, workspace_id: str) -> int:
    ...
    if hasattr(store, "_database"):
        with store._database.snapshot() as snapshot:
            return spanner_pause_epoch(snapshot, store._param_types, workspace_id)
    return int(store._run_transaction(lambda conn: postgres_pause(conn, workspace_id)[1]))
...
def reject_postgres_reservation(conn: Any, store: Any, reservation_id: str) -> None:
    ...
    reservation = store._read_entity_tx(
        conn, _RESERVATION_KIND, reservation_id, Reservation, for_update=True
    )
    ...
    if won:
        released = conn.execute(
            "UPDATE tr_credit_balance SET reserved = reserved - %s "
