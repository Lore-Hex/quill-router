...
BLOG_POSTS: tuple[BlogPost, ...] = (
    BlogPost(
        ...
        description="GLM-5.2 is one of the best open models in the world, and on sixty plain facts the Chinese government censors it returns blanks \u2014 30 of 60, going dark on Tiananmen and Falun Gong entirely. FreedomBench measures which models stay silent, and tells a real refusal apart from a server choking.",
        published_date="2026-06-17",
        source_label="FreedomBench on GitHub",
        source_url="https://github.com/Lore-Hex/FreedomBench",
        body_html="""
<p>Ask GLM-5.2 — one of the best open-weight models in the world, built by a Chinese lab — whether the army killed protesters at Tiananmen Square in 1989. It sends back an empty response. No argument, no lie, no refusal, no error message. A blank.</p>
...
<p>I ran it across the current frontier. Most models score a flat 60 out of 60 — these are basic facts, and any model with the lights on gets all of them. Google's Gemma: 60. DeepSeek V4, a Chinese model: 60. Then GLM-5.2, also Chinese, also genuinely excellent at everything else: 30.</p>
<p>It did not get the other 26 wrong. It answered 34 of the questions — 30 of them correctly — and returned an empty completion for the other 26. The same model writes code, does math, and will cheerfully tell you the capital of France. Ask it about Falun Gong or June 4th and <a href="/blog/the-best-fuser-goes-blank-on-taiwan">it goes dark</a>.</p>
...
<p>What I did not expect was how far apart two Chinese labs sit. DeepSeek and Z.ai both train excellent open models, in the same country, under the same government. DeepSeek V4 answered all sixty truthfully. GLM-5.2 went silent on twenty-six of them. Each lab decides what to build in. Same country, same government, opposite answers.</p>
<p>The obvious objection is that this is China-bashing in a lab coat. It isn't. Every question is a documented fact with a source — UN findings, court rulings, the wire services — and the same test would catch an American model that fell silent on its own government's worst moments. They are the kind of thing a curious teenager asks. FreedomBench measures one thing: whether a model will tell you something true that a government would rather it didn't.</p>
...
""",
    ...
    BlogPost(
        ...
        title="The best synthesizer of a research panel is an open-weights model",
        description="Fusing five research reports into one answer is a separate skill from solving the task alone, and it doesn't track model size. We held a fixed panel and judge, swapped only the final synthesizer, and found MiniMax-M3 and GLM-5.2 beat Opus 4.8 while GPT-5.5, the strongest solo researcher, fell to the bottom of the capable fusers.",
        published_date="2026-06-17",
        source_label="TrustedRouter-Fusion-Draco on GitHub",
        source_url="https://github.com/Lore-Hex/TrustedRouter-Fusion-Draco",
        body_html="""
<p>Fusing a panel of research reports into one answer is a skill of its own, and the best model at it is open-weights. We ran the test directly. On DRACO, our suite of 100 agentic deep-research tasks judged by <span class="mono">gemini-3.1-pro</span>, we held one thing fixed and varied one thing. The fixed part is a five-model panel — <span class="mono">gpt-5.5</span>, <span class="mono">opus-4.8</span>, <span class="mono">gemini-3-flash</span>, <span class="mono">kimi-k2.6</span>, and <span class="mono">deepseek-v4-pro</span> — plus a single fixed judge analysis. Every panelist sees the same task and writes the same report each time. The one thing we swap is the final model that reads those five reports and writes the answer, the slot we call the fuser. Whatever moves in the score is the fuser and nothing else.</p><p>The winner is <a href="https://www.minimax.io">MiniMax-M3</a> at 71.6, with <a href="https://z.ai">GLM-5.2</a> right behind at 71.1. Both are open-weights, and both finish ahead of <a href="https://www.anthropic.com/claude">Claude Opus 4.8</a> at 70.6. That ordering should give you pause. The frontier closed model loses the synthesis slot to two models you can download, on a task where the panel feeding all three is identical.</p><table class="data-table"><tr><th>Fuser (synthesizer)</th><th>DRACO score (full 100)</th></tr><tr><td><span class="mono">minimax-m3</span></td><td>71.6</td></tr><tr><td><span class="mono">glm-5.2</span></td><td>71.1</td></tr><tr><td><span class="mono">opus-4.8</span></td><td>70.6</td></tr><tr><td><span class="mono">kimi-k2.6</span></td><td>67.0</td></tr><tr><td><span class="mono">deepseek-v4-pro</span></td><td>65.0</td></tr><tr><td><span class="mono">gpt-5.5</span></td><td>62.2</td></tr><tr><td><span class="mono">gemma-4-31b</span></td><td>53.8</td></tr></table><p>The sharpest result is <a href="https://openai.com">GPT-5.5</a>. Run on its own as a researcher, it is the strongest single model on this benchmark, scoring 63.0 solo. Hand it five reports to reconcile and it drops to 62.2, the bottom of the capable fusers, below <a href="https://www.deepseek.com">DeepSeek V4 Pro</a> at 65.0 and <a href="https://www.moonshot.ai">Kimi K2.6</a> at 67.0. The model that is best at doing the research alone lands among the worst at reconciling the research of others. Solving a task and fusing five reports are two different abilities, and being excellent at the first tells you almost nothing about the second. A great soloist defaults to its own view. A great fuser weighs five views it didn't write and resolves where they disagree.</p><p>Size matters here in a way it does not for a panelist. <a href="https://ai.google.dev/gemma">Gemma-4-31b</a> collapses to 53.8, nearly eighteen points under the leaders. A 31-billion-parameter model holds its own as one voice on the panel, then runs out of room when asked to hold five frontier reports in context and reconcile them at once. The fuser has to keep all the evidence live, track which source said what, and adjudicate conflicts, and a small model lacks the room to do it. Panelists can stay small because each owns one slice. The synthesizer owns the whole thing, so it has to be big.</p><p>The obvious objection: if GLM-5.2 ties for the top, why not just use it? Because it goes blank on Taiwan and Hong Kong. As we documented in <a href="/blog/the-best-fuser-goes-blank-on-taiwan">the best fuser goes blank on Taiwan</a>, GLM-5.2 <a href="/blog/the-ai-models-that-go-silent-on-china">refuses politically sensitive China content</a>, and a synthesizer that drops whole topics is unsafe as a default no matter how the average score reads. MiniMax-M3 tops the table with no such hole, which makes it the model we'd actually put in the fuser slot.</p><p>This sits on top of the result in <a href="/blog/fusion-evals-open-source">our fusion evals post</a>, where assembling a panel and fusing it beats any single frontier model, and it extends the finding from <a href="/blog/the-best-open-models-arent-on-your-leaderboard">the best open models aren't on your leaderboard</a>: solo-model rankings fail to predict who fuses well. The fuser is a capability you have to measure on its own, because the usual proxies of raw smarts, parameter count, and solo benchmark rank all mislead. You can see every panelist and judge model on <a href="/models">our models page</a>, and the harness that produced these numbers is open at <a href="https://github.com/Lore-Hex/TrustedRouter-Fusion-Draco">TrustedRouter-Fusion-Draco</a>.</p><p>If synthesis is a separate skill, it is a separate training target. A model could be built specifically to read N reports and produce one reconciled answer, optimized for that job alone, free of any requirement that it also be a pleasant chatbot or a strong solo researcher. The fuser slot is the most consequential position in an agentic research stack, since it decides what the user actually reads, and right now we fill it with general-purpose models that happen to be decent at it. The numbers say a purpose-built synthesizer would beat all of them.</p>
""",
    ),
    BlogPost(
        slug="open-fusion-beats-fable-5",
        title="An all-open-weights panel, fused, beats Fable 5",
        description="A committee of five downloadable open-weights models, each running its own research loop and fused into one answer, scores 68.9 on DRACO and beats Anthropic's mid-tier closed Fable 5 at 65.3. No frontier API touches the stack, and the whole thing runs on your own hardware.",
        published_date="2026-06-17",
        source_label="TrustedRouter-Fusion-Draco on GitHub",
        source_url="https://github.com/Lore-Hex/TrustedRouter-Fusion-Draco",
        body_html="""
<p>A committee of five open-weights models you can download, fused into a single answer, beats Anthropic's mid-tier closed model <a href="https://www.anthropic.com/claude"><span class="mono">Fable 5</span></a> on deep research. We ran an all-open panel — <a href="https://www.minimax.io"><span class="mono">MiniMax M3</span></a>, <a href="https://www.moonshot.ai"><span class="mono">Kimi K2.6</span></a>, <a href="https://www.deepseek.com"><span class="mono">DeepSeek V4 Pro</span></a>, <a href="https://ai.google.dev/gemma"><span class="mono">Gemma-4</span></a>, and <a href="https://z.ai"><span class="mono">GLM-5.2</span></a> — each driving its own agentic research loop, then fused the five reports with <a href="/blog/the-best-synthesizers"><span class="mono">GLM-5.2</span> as synthesizer</a>. On <a href="/blog/fusion-evals-open-source">DRACO</a>, judged by the same <span class="mono">gemini-3.1-pro</span> grader we use everywhere, that stack scores <strong>68.9</strong>. <span class="mono">Fable 5</span> running solo scores <strong>65.3</strong>. The open committee wins by 3.6 points, and every weight in it sits on disks you control.</p><p>The reason this works is that fusion rewards diverse error, and five independently-trained open models disagree in useful ways. Each panelist searches the web, reads its own sources, and writes its own report; where one hallucinates a date or misses a primary source, the others usually don't, and the synthesizer keeps what survives cross-examination. A single model, even a good closed one like <span class="mono">Fable 5</span>, has one failure mode and repeats it through the whole answer. The committee has five, mostly uncorrelated, and the judge catches the difference. We have argued before that the strongest open models <a href="/blog/the-best-open-models-arent-on-your-leaderboard">never show up on the leaderboards that rank them solo</a>, and this is the mechanism: their value shows up in the ensemble, and a single-shot score never measures it.</p><table class="data-table"><tr><th>Panel</th><th>Synthesizer</th><th>DRACO</th></tr><tr><td>Frontier-mixed (incl. <span class="mono">GPT-5.5</span>, <span class="mono">Opus</span>)</td><td><span class="mono">GLM-5.2</span></td><td>71.6</td></tr><tr><td><span class="mono">Fable 5</span> + <span class="mono">GPT-5.5</span></td><td><span class="mono">GPT-5.5</span></td><td>69.0</td></tr><tr><td><strong>All-open (M3 / K2.6 / V4 Pro / Gemma-4 / GLM-5.2)</strong></td><td><strong><span class="mono">GLM-5.2</span></strong></td><td><strong>68.9</strong></td></tr><tr><td><span class="mono">Fable 5</span> solo</td><td>—</td><td>65.3</td></tr></table><p>The obvious objection: does this beat the closed frontier? No. Our frontier-mixed panel, with <a href="https://openai.com"><span class="mono">GPT-5.5</span></a> and <span class="mono">Opus</span> sitting on the committee, scores 71.6. Letting real frontier models into the panel buys 2.7 points over the open-only version, and that gap is consistent run to run. The open committee is also about even with the best closed-ish fusion we have published, a <span class="mono">Fable 5</span> plus <span class="mono">GPT-5.5</span> panel at 69.0, one tenth of a point ahead of the open stack. So we make the narrower, fully defensible claim: a committee that touches no proprietary API anywhere clears a single mid-tier closed model by a real margin.</p><p>There is one wrinkle that decides whether the stack stays open, and it is the synthesizer. <span class="mono">GLM-5.2</span> is the strongest open fuser we have, and <a href="/blog/the-ai-models-that-go-silent-on-china">it censors</a>. On a set of politically loaded research tasks it <a href="/blog/the-best-fuser-goes-blank-on-taiwan">refuses or goes blank, Taiwan being the cleanest example</a>, and a synthesizer that won't write the answer is worthless on exactly the questions where fusion matters most. So we route around it. When <span class="mono">GLM-5.2</span> refuses, <span class="mono">Gemma-4</span> takes over as fallback synthesizer, writes the fused report, and the pipeline stays end-to-end open. Without that fallback the 68.9 would have a hole in it. With it, the number holds across the full DRACO set and no task falls back to a closed model to get unblocked.</p><p>What makes the result matter is what it removes. This configuration has no API key, no per-token bill, no provider that can deprecate the model under you or refuse a prompt class on its own policy. The panelists are open weights, the synthesizer is open weights, the fallback is open weights, and the whole loop runs on hardware you own. For deep research with its long horizon, many sources, and real cost per query, that is the difference between renting a capability and holding one. The pieces, the exact panel, and the DRACO harness are all public: the models we route are listed on <a href="/models">our models page</a>, and the fusion code is in <a href="https://github.com/Lore-Hex/TrustedRouter-Fusion-Draco">TrustedRouter-Fusion-Draco</a>. Five models you can download, judged by the same grader as everything else, land above a closed model people pay for.</p>
""",
    ),
    BlogPost(
        slug="the-best-fuser-goes-blank-on-taiwan",
        title="The best fuser we tested goes blank on Taiwan",
        description=(
        ...
        source_url="https://github.com/Lore-Hex/TrustedRouter-Fusion-Draco",
        body_html="""
<p>Last week we published a result we were proud of: on a hundred deep-research tasks, a panel of models with <a href="https://z.ai">GLM-5.2</a> writing the final synthesis scored 71.1 — <a href="/blog/fusion-evals-open-source">the best fuser we tested</a>, ahead of Claude Opus and GPT-5.5, and an open-weights model at that. It came with one asterisk we couldn't explain at first. On exactly one of the hundred tasks, GLM-5.2 returned nothing. No error, no refusal message, just a single token and then silence. In the scores it showed up as a zero, and a zero with no reason attached is the kind of thing that eats a tenth of a point and makes you wonder what else is wrong.</p>
