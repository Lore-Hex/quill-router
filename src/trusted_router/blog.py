...
BLOG_POSTS: tuple[BlogPost, ...] = (
    BlogPost(
        ...
        body_html="""
<p>Last week we published a result we were proud of: on a hundred deep-research tasks, a panel of models with <a href="https://z.ai">GLM-5.2</a> writing the final synthesis scored 71.1 — <a href="/blog/fusion-evals-open-source">the best fuser we tested</a>, ahead of Claude Opus and GPT-5.5, and an open-weights model at that. It came with one asterisk we couldn't explain at first. On exactly one of the hundred tasks, GLM-5.2 returned nothing. Not an error, not a spoken refusal — a single token and then silence. In the scores it showed up as a zero, and a zero with no reason attached is the kind of thing that quietly eats a tenth of a point and makes you wonder what else is wrong.</p>
<p>So we dug in, because a model that blanks once in a hundred is a model you can't fully trust the other ninety-nine times. It wasn't length: the input was under 20,000 tokens, nothing for a model that holds far more. It wasn't our code trimming the output — the raw response was genuinely empty, one token long. And it wasn't the question; hand GLM-5.2 that same task on its own and it writes a thorough report. The blank appeared only when we gave it the panel's evidence to synthesize.</p>
