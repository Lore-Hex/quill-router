...
BLOG_POSTS: tuple[BlogPost, ...] = (
    BlogPost(
        ...
        body_html="""
<p>Ask GLM-5.2 — one of the best open-weight models in the world, built by a Chinese lab — whether the army killed protesters at Tiananmen Square in 1989. It doesn't argue with you, and it doesn't lie to you. It sends back an empty response. No text, no refusal, no error message. A blank.</p>
...
<p>What I did not expect was how far apart two Chinese labs sit. DeepSeek and Z.ai both train excellent open models, in the same country, under the same government. DeepSeek V4 answered all sixty truthfully. GLM-5.2 went silent on twenty-six of them. Where a model is trained doesn't decide whether it's censored. Each lab decides what to build in.</p>
<p>The obvious objection is that this is China-bashing in a lab coat. It isn't. Every question is a documented fact with a source — UN findings, court rulings, the wire services — and the same test would catch an American model that fell silent on its own government's worst moments. These aren't gotchas; they are the things a curious teenager asks. FreedomBench doesn't measure whether a model dislikes China. It measures whether a model will tell you something true that a government would rather it didn't.</p>
