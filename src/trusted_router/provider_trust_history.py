...
PROVIDER_ENVIRONMENTS = {"paypal": frozenset({"sandbox", "live"}), "adyen": frozenset({"test", "live"})}
...
def provider_marker_qualifies(
    ...
    """
    if provider not in PROVIDER_SOURCES or not account_id or environment not in PROVIDER_ENVIRONMENTS[provider]:
        return False
...
def provider_scan(
    ...
    payment_rows: dict[tuple[str, str | None], TrustEvent] = {}
    for payment_row in payments:
        key = (payment_row.provider, payment_row.original_payment_ref)
        if key in payment_rows and payment_rows[key] != payment_row:
            raise ValueError("conflicting canonical payment")
        payment_rows[key] = payment_row
    ...
    for event in adverse:
        ...
        if old is not None:
            ...
            if outcome == "replay":
                ...
                if event.provider_ordering_watermark > old.provider_ordering_watermark:
                    latest[key] = event
                continue
        latest[key] = event
    ...
    return StripeTrustScan(tuple(payment_rows.values()), tuple(latest.values()),
                           tuple([*payment_rows.values(), *source_adverse]),
                           tuple(outstanding), tuple(sorted(set(unmatched))),
                           credit_evidence={str(p.original_payment_ref): (p.event_id,)
                                            for p in payment_rows.values()})
