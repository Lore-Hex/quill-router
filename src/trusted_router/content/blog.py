...
<p>Keys are per-boot and per-instance, while their attestations re-mint during that boot. The feature: no long-lived signing key exists anywhere, so there is nothing to exfiltrate and rotate. The consequence: a receipt must outlive both the attestation version and the instance that signed it, so every observed re-mint lands in an <a href="/.well-known/inference-receipt-keys">append-only public key log</a>, commitment-checked before recording and fully chain-verified where supported, and alarmed if a key id ever reappears with different material. Your receipt from today still verifies after newer evidence replaces the instance's current document or that instance is gone.</p>
