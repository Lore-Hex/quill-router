...
def first_party_hosts(domains: list[str] | tuple[str, ...]) -> list[str]:
    ...
    return [host for domain in normalized for host in (domain, f"*.{domain}")]


def _is_first_party_host(host: str, domains: set[str]) -> bool:
...
def rewrite_url_map(
    ...
    for rule in result.get("hostRules", []):
        ...
        retained = [host for host in hosts if not _is_first_party_host(host, domain_set)]
        if retained:
            rewritten = dict(rule)
    ...
    for test in result.get("tests", []):
        ...
        if _is_first_party_host(host, domain_set):
