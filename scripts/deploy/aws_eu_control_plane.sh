...
# and AWS has never published a source_commit, let alone one carrying that
# file. Enforcing today would stop every AWS control-plane deploy for a reason
# nobody can clear from this repository.
#
...
# it arms this script, azure_control_plane.sh and deploy.yml together.
#
...
# envelope needs its enclave to keep v2 read support permanently. The way past
# this gate is to fix the ordering, not to pass an argument.
...
NOTRUN
  ...
  echo "the AWS enclave cannot read. Nothing has been built or pushed." >&2
  echo "" >&2
