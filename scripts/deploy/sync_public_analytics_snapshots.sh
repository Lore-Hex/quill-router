...
gc compute ssh "$NAME" \
  ...
  --command="sudo sh -c '
    ...
    count=\$(/usr/bin/clickhouse-client --user tr --password \"\$CH_PASSWORD\" \
      --database tr --query \"SELECT uniqExact(name) FROM public_analytics_snapshots FINAL WHERE generated_at >= now() - INTERVAL 10 MINUTE AND name IN ('\''leaderboard'\'', '\''leaderboard_evidence'\'', '\''apps'\'', '\''video_leaderboard'\'', '\''status_inputs'\'', '\''client_reliability'\'') FORMAT TSVRaw\")
    if [ \"\$count\" != 6 ]; then
      rollback
      exit 1
    fi
...
log "public analytics snapshot worker is current and publishing all six products"
