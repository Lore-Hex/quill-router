...
gc run jobs "$mutation" "$JOB_NAME" \
  ...
  --update-secrets="TR_SENTRY_DSN=trustedrouter-sentry-dsn:latest,TR_PAYPAL_CLIENT_ID=trustedrouter-paypal-client-id:latest,TR_PAYPAL_CLIENT_SECRET=trustedrouter-paypal-client-secret:latest,TR_PAYPAL_WEBHOOK_ID=trustedrouter-paypal-webhook-id:latest" \
  --max-retries=0 \
  --task-timeout=14m \
  --cpu=1 \
  --memory=512Mi \
