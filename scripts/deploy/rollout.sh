...
if [ "${TR_DEPLOY_RECONCILE_LB:-1}" != 1 ]; then
  ...
  echo "ERROR: six-surface staging cannot set edge_header without verified LB reconciliation" >&2
  exit 1
fi
...
surface_runtime_contract() {
  ...
  case "$surface" in
    ...
    webhooks)
      ...
      REQUEST_BODY_READ_TIMEOUT_SECONDS=10; DEFAULT_URL_DISABLED=true ;;
    internal)
      CONCURRENCY=8; MIN_INSTANCES=2; MAX_INSTANCES=50; TIMEOUT_SECONDS=300
      MEMORY=2Gi; SPANNER_POOL_SIZE=8; MAX_REQUEST_BODY_BYTES=33554432
...
surface_env_vars() {
  local surface="$1"
  surface_runtime_contract "$surface"
  SURFACE_ENV=(
