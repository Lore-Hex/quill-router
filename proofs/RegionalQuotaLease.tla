...
(*     structure, not magnitude. int64 overflow is a separate concern, and   *)
(*     this line used to claim it was "covered by the Python property        *)
(*     tests". It is NOT covered, by this model or by anything else. The     *)
(*     module has exactly one property test                                  *)
(*     (tests/test_regional_quota_leases.py, the `@given` at line 229); it   *)
(*     caps a grant at 1,000,000 microdollars and each reservation at        *)
(*     100,000, and the largest literal anywhere in that file is             *)
(*     10,000,003 — twelve orders of magnitude below an int64 boundary.      *)
(*     The sentence is corrected rather than deleted: see scope limit 8.     *)
...
(*      requires does not exist in the code, and nothing in the module says  *)
(*      it has to. `fencing_token` appears on seventeen LINES of src/ —      *)
(*      eighteen occurrences of the identifier, because `if fencing_token != *)
(*      self.fencing_token:` carries two — and not one of them writes it.    *)
(*      They are: the field declaration, the `<= 0` check in `__post_init__` *)
(*      and its message, a parameter taking a caller-supplied token in, a    *)
(*      `self._require_fence(fencing_token)` call passing one on, and the    *)
(*      two sides of the comparison inside `_require_fence`. There is no     *)
(*      assignment to it and no increment of it anywhere in src/: `close()`  *)
(*      returns `replace(self, state=LeaseState.CLOSED)` with the token      *)
(*      untouched, and `begin_drain()` and `quarantine()` do the same for    *)
(*      their states. So regional_quota_leases.py today IS the mutant in the *)
...
(*      against.                                                             *)
(*                                                                           *)
(*      That has cost nothing so far because the module is dark and          *)
(*      unfinished on both ends: `config.regional_quota_leases_enabled`      *)
(*      defaults to False and refuses to be set outside local/test, nothing  *)
(*      in src/ imports the module — those config flags are its only         *)
(*      mention anywhere else in src/ — so it has no service callers, and    *)
(*      neither the granter nor the forced reclaimer has been written. The   *)
(*      actionable claim is therefore about code that does not exist yet:    *)
(*      whoever writes the granter and the reclaimer must advance the token  *)
(*      in the transaction that closes or supersedes a lease, and make that  *)
(*      transaction the only writer of it — and no docstring, comment, test  *)
(*      or type in the module says so. The change is NOT made in this        *)
(*      commit; this file's job is only to establish that it is needed.      *)
(*                                                                           *)
(*      CORRECTION (round 5). This paragraph used to say that "nothing in    *)
(*      the tree today would tell an implementer that it has to" advance,    *)
(*      and that "this paragraph is the only place it is written down". Both *)
(*      sentences overstated the gap and are withdrawn. The design doc,      *)
(*      docs/design/regional-quota-leases.md, lists "a monotonically         *)
(*      increasing fencing token" at line 24 among what every lease has;     *)
(*      declares `fencing_token INT64 NOT NULL` in the schema sketch at line *)
(*      73; has the grant write the token in step 2 (lines 39-40); and has   *)
(*      the reconciler close the lease "in one fenced operation" in step 5   *)
(*      (lines 43-44). An implementer reading that IS told the token is      *)
(*      meant to increase. What neither the design doc nor the module says   *)
(*      is that the transaction which CLOSES or SUPERSEDES a lease is the    *)
(*      one that must advance it, and that it must be the only writer of it  *)
(*      — which is the FIRST conjunct of (b), not the whole of it. The       *)
(*      second conjunct, comparing the presented token inside the settling   *)
(*      transaction, is written down in both sources. The half that is       *)
(*      missing from both is which transaction advances the token, and       *)
(*      that it must have a sole writer.                                     *)
(*                                                                           *)
...
(*                                                                           *)
(*   8. Nothing here or in the Python bounds MAGNITUDE. Every amount in      *)
(*      this model is a small natural; on the Python side `__post_init__`    *)
(*      requires a positive token and a positive grant and enforces          *)
(*      `accounted <= granted`, but places no ceiling on any amount, and     *)
(*      the one property test's ceilings are the ones quoted under WHAT IS   *)
(*      ABSTRACTED. An int64 overflow in the durable counters — a grant,     *)
(*      an escrow total, or a sum of holds crossing 2^63 — is therefore      *)
(*      unchecked by this spec, by that test, and by the module. There is    *)
(*      a `regional_quota_lease_max_microdollars` setting (10,000,000 by     *)
(*      default), but config.py only validates that it is positive and no    *)
(*      code reads it, so it bounds nothing today.                           *)
(***************************************************************************)
