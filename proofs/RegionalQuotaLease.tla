...
(*      requires does not exist, and nothing in the tree today would tell an *)
(*      implementer that it has to. `fencing_token` appears on seventeen     *)
(*      LINES of src/ — eighteen occurrences of the identifier, because      *)
(*      `if fencing_token != self.fencing_token:` carries two — and not one  *)
(*      of them writes it. They are: the field declaration, the `<= 0`       *)
(*      check in `__post_init__` and its message, a parameter taking a       *)
(*      caller-supplied token in, a `self._require_fence(fencing_token)`     *)
(*      call passing one on, and the two sides of the comparison inside      *)
(*      `_require_fence`. There is no assignment to it and no increment of   *)
(*      it anywhere in src/: `close()` returns                               *)
(*      `replace(self, state=LeaseState.CLOSED)` with the token untouched,   *)
(*      and `begin_drain()` and `quarantine()` do the same for their states. *)
(*      So regional_quota_leases.py today IS the mutant in the "keep the     *)
(*      fence, delete the token bump" row — the fence is honoured on every   *)
(*      call and the hold from the retired generation settles anyway,        *)
(*      because its token still matches the row it is compared against.      *)
(*                                                                           *)
...
(*      or type in the module says so. This paragraph is the only place it is*)
(*      written down. The change is NOT made in this commit; this file's job *)
(*      is only to establish that it is needed.                              *)
(*                                                                           *)
